...
export default function Projects() {
  ...
  return (
    <section id="projects" className="bg-surface" aria-labelledby="projects-heading">
      <div className="section-pad">
        <SectionHeader
          number="04"
          title={lang === 'ar' ? 'المشاريع' : lang === 'ru' ? 'Проекты' : 'Projects'}
          subtitle={lang === 'ar' ? 'أعمال قمت بتطويرها' : lang === 'ru' ? 'Проекты, которые я разработал' : 'Projects I have built'}
        />

        <div ref={ref} className="reveal grid grid-cols-1 md:grid-cols-2 gap-6">
          {projects.map((proj) => {
            const techs = proj.technologies || proj.technologiesEn || [];
            const projName = lang === 'ar' ? proj.nameAr : lang === 'ru' ? proj.nameRu : proj.nameEn;
            ...
            return (
              ...
              >
                ...
                <div className="aspect-video w-full bg-[rgb(var(--accent)/0.05)] border-b border-[rgb(var(--border))] flex items-center justify-center overflow-hidden">
                  {hasImages ? (
                    <ProjectGallery images={proj.images} alt={projName} />
                  ) : (
                    <div className="flex flex-col items-center gap-2 text-foreground-muted py-8">
                      <Hammer size={26} aria-hidden="true" />
                      <span className="text-xs font-medium">
                        {lang === 'ar' ? 'قيد التطوير — الصور قريبًا' : lang === 'ru' ? 'В разработке — скоро скриншоты' : 'In progress — screenshots coming soon'}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="p-6 flex flex-col flex-1">
                  {proj.featured && (
                    <span className="text-xs font-medium text-[rgb(var(--accent))] mb-2 uppercase tracking-wide">
                      {lang === 'ar' ? 'مشروع مميز' : lang === 'ru' ? 'Избранный проект' : 'Featured Project'}
                    </span>
                  )}
                  ...
                  <p className="text-sm text-foreground-muted leading-relaxed mb-4 flex-1">
                    {lang === 'ar' ? proj.descriptionAr : lang === 'ru' ? proj.descriptionRu : proj.descriptionEn}
                  </p>
                  {techs.length > 0 && (
                  ...
                  {proj.githubUrl && (
                    ...
                    >
                      {lang === 'ar' ? 'عرض على GitHub' : lang === 'ru' ? 'На GitHub' : 'View on GitHub'} →
                    </a>
                  )}
                </div>
              </div>
